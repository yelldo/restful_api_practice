...
public class ClassUtils{
	...
	public static List<Class<?>> getClasses(String packageName) {
		...
		try {
			String pname = packageDirName.substring(1);
				...
				if ("file".equals(protocol)) {
					...
					try {
						...
						while (entries.hasMoreElements()) {
							...
							// 如果前半部分和定义的包名相同
							// log.info("NAME3:" + name + "," + packageDirName +
							...
							if (name.startsWith(packageDirName)) {
								...
								if (idx != -1) {
									// 获取包名 把"/"替换成"."
									packageName = name.substring(1, idx)
											.replace('/', '.');
								}
								// 如果可以迭代下去 并且是一个包
								if ((idx != -1) || recursive) {
		...
		try {
			...
			while (dirs.hasMoreElements()) {
				...
				if ("file".equals(protocol)) {
					...
					String filePath = URLDecoder.decode(url.getFile(), "UTF-8");
					// 以文件的方式扫描整个包下的文件 并添加到集合中
					findAndAddClassesInPackageByFile2(packageName, filePath,
							recursive, classes);
				} else if ("jar".equals(protocol)) {
					// 如果是jar包文件
					// 定义一个JarFile
							...
							if (name.startsWith(packageDirName)) {
								...
								if ((idx != -1) || recursive) {
									...
									if (!name.endsWith(".class")
										 ...
										 * e.printStackTrace(); }
										 */
										URL url2 = org.apache.commons.lang3.ClassUtils.class
												.getResource(name);
										// log.info("URLADD2:" + name + "," +
										// url2);
										if (url2 == null) {
											log.error("Not found url2 " + url2);
										} else {
											classes.add(url2);
										}
									}
			...
			log.info("DIRS:" + packageDirName + ",all:" + alls);
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	public static void findAndAddClassesInPackageByFile2(String packageName,
		...
		for (File file : dirfiles) {
			// 如果是目录 则继续扫描
			if (file.isDirectory()) {
				findAndAddClassesInPackageByFile2(
						packageName + "." + file.getName(),
						file.getAbsolutePath(), recursive, classes);
			} else {
				// 如果是java类文件 去掉后面的.class 只留下类名
				/*
				 * String className = file.getName().substring(0,
				 * file.getName().length() - 6); try { //添加到集合中去
				 * classes.add(Class.forName(packageName + '.' + className)); }
				 * catch (ClassNotFoundException e) { e.printStackTrace(); }
				 */
				String mstr = "/" + packageName.replace(".", "/") + '/'
						+ file.getName();
				URL url = org.apache.commons.lang3.ClassUtils.class.getResource(mstr);
				classes.add(url);
			}
		}
	}
