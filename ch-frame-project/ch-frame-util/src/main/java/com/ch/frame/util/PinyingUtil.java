...
public class PinyingUtil {
    ...
    public static char[] getHeadByChar(char src, boolean isCapital) {
        ...
        if (pinyingStr == null) return null;
        //创建返回对象
    ...
    public static String getHeadByString2(String src) {
        ...
        for (char c : src.toCharArray()) {
            try {
                String[] ps = getHeadByString(c + "");
                if (ps == null || ps.length < 1 || ps[0] == null) continue;
                if (s == null)
                    s = ps[0];
                else
                    s = s + ps[0];
    ...
    public static String stringToShortPinyin(String chinese) {
        if (chinese == null) {
            return null;
        }
        ...
        try {
            ...
            for (int i = 0; i < chinese.length(); i++) {
                ...
                String pinyin = String.valueOf(array[0].charAt(0));
                sb.append(pinyin);
            }
            return sb.toString();
        } catch (BadHanyuPinyinOutputFormatCombination e) {
    ...
    public static void main(String[] args) {
        System.out.println(Arrays.asList(getHeadByString("是否大写")));
    }
